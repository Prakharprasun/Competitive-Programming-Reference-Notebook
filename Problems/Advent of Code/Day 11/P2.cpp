...
map <pair<int,i128>,int> dp;
i128 helper(int turns, i128 i) {
    if (dp.find({turns,i})!= dp.end()) {
        return dp[{turns,i}];
    }
    if (turns == 0) return 1;
    {
        {
            ...
            if (c % 2 ==0) {
                dp[{turns,i}] = helper (turns-1, i/binpow(10,c/2)) + helper (turns -1, i%binpow(10,c/2));
                return dp[{turns,i}];
            }
            else {
                dp[{turns,i}] = helper (turns-1, i*2024);
                return dp[{turns,i}];
            }
        }
    }
